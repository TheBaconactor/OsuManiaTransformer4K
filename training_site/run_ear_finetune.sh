...
echo "[1/3] Build Beat This dataset from Datasets2 (dataset=${DATASET_NAME}, val_artist=${VAL_ARTIST})"
"${PYTHON_BIN}" scripts/build_beat_this_osu_dataset.py \
  --dataset-name "${DATASET_NAME}" \
  --val-artist "${VAL_ARTIST}" \
  --spect-dtype float32

echo "[2/3] Fine-tune Beat This (tune=${TUNE}, epochs=${EPOCHS}, batch=${BATCH_SIZE}, gpu=${GPU})"
"${PYTHON_BIN}" scripts/finetune_beat_this_osu.py \
