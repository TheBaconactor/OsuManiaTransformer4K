"""
Build a Beat This!-style training dataset from this repo's `Datasets2/`.
...
This script converts `Datasets2/annotations/*.json` + `Datasets2/audio/*.mp3` into
...
def main(argv: Iterable[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build a Beat This dataset from Datasets2.")
    parser.add_argument(
        "--datasets2",
        type=str,
        default=str(REPO_ROOT / "Datasets2"),
        help="Path to Datasets2 folder (contains audio/ and annotations/).",
    )
    parser.add_argument(
        "--out",
        type=str,
    ...
    if not items:
        print("[ERROR] No valid items found in Datasets2.", file=sys.stderr)
        return 2
