...
Input: .osz files in osu! Exports folder (Windows)
Output: Organized data in data/datasets2/ with audio, annotations, and metadata
...
_default_exports = None
try:
    local_appdata = os.environ.get("LOCALAPPDATA")
    if local_appdata:
        _default_exports = str(Path(local_appdata) / "osu!" / "Exports")
except Exception:
    _default_exports = None
DEFAULT_OSZ_INPUT_DIR = Path(os.environ.get("OSU_EXPORTS_DIR") or _default_exports or r"C:/Users/troll/AppData/Local/osu!/Exports")
DEFAULT_DATASET_OUTPUT_DIR = Path(__file__).resolve().parent
