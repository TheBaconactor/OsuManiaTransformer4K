...
def load_audio(path, dtype="float64"):
    try:
        ...
        try:
            ...
            try:
                # `soundfile` often can't read MP3 on Windows; `miniaudio` provides a tiny decoder.
                ...
                if int(decoded.nchannels) > 1:
                    samples = samples.reshape(-1, int(decoded.nchannels))
                return np.asanyarray(samples, dtype=dtype), int(decoded.sample_rate)
            except Exception:
