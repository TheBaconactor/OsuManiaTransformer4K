...
wsl bash -lc "set -euo pipefail; mkdir -p $WslRepo; rsync -a --delete --exclude '.git/' --exclude '.venv/' --exclude '__pycache__/' --exclude 'reports/' --exclude 'modules/beat_this/data/' --exclude 'modules/beat_this/checkpoints/' '$RepoRootWsl/' '$WslRepo/'"
...
wsl bash -lc "set -euo pipefail; cd $WslRepo; source .venv/bin/activate; ONNX_NAME='$OnnxName' VAL_ARTIST='$ValArtist' EPOCHS='$Epochs' BATCH_SIZE='$BatchSize' TRAIN_LENGTH='$TrainLength' GPU='auto' ./training/wsl/run_ear_finetune.sh"
