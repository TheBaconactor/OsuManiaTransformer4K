...
```
If you want to use the DBN for postprocessing, add `--dbn`. The DBN parameters are the default ones from madmom. This requires installing the `madmom` package. On Windows, the most reliable setup is usually a dedicated Conda env (Python 3.10) and `pip install "madmom @ git+https://github.com/CPJKU/madmom.git"` (see `docs/madmom_dbn_setup.md`).
