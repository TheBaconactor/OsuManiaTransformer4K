...
def _checkpoint_map_location(device: str | torch.device) -> str | torch.device:
    """
    Determine a safe map_location for loading checkpoints.

    Some backends (notably torch-directml / privateuseone) cannot be used as a
    torch.load map_location target; load onto CPU first and move the model after.
    """
...
def _patch_rotary_embedding_for_directml() -> None:
    """
    Work around torch-directml limitations with non-contiguous views.

    rotary-embedding-torch's apply_rotary_emb uses tensor views and torch.cat().
    On DirectML this can fail with a generic "The parameter is incorrect."
    error; making the cat inputs contiguous avoids that in practice.
    """
    try:
    ...
    if getattr(rotary_mod, "_directml_contiguous_patch", False):
        return

    def apply_rotary_emb_directml(
        freqs,
        t,
        start_index=0,
        scale=1.0,
        ...
        if not rotary_mod.exists(freqs_seq_dim):
            if freqs.ndim == 2 or t.ndim == 3:
                freqs_seq_dim = 0

        if t.ndim == 3 or rotary_mod.exists(freqs_seq_dim):
            seq_len = t.shape[seq_dim]
            freqs = rotary_mod.slice_at_dim(
                freqs, slice(-seq_len, None), dim=freqs_seq_dim
            )
        ...
        t_transformed = (t_middle * freqs.cos() * scale) + (
            rotary_mod.rotate_half(t_middle) * freqs.sin() * scale
        )

        out = torch.cat(
            (t_left.contiguous(), t_transformed.contiguous(), t_right.contiguous()),
            dim=-1,
        )
        return out.type(dtype)

    rotary_mod.apply_rotary_emb = apply_rotary_emb_directml
    rotary_mod._directml_contiguous_patch = True


def _refine_peak_times(
    logits: torch.Tensor, times_s: np.ndarray, fps: int = 50
) -> np.ndarray:
    """
    Sub-frame peak refinement by quadratic interpolation around local maxima.

    `times_s` must be aligned to the same FPS grid as `logits` (default: 50FPS).
    """
    times_s = np.asarray(times_s, dtype=np.float64)
    if times_s.size == 0:
        return times_s

    prob = logits.detach().double().sigmoid().cpu().numpy()
    n = int(prob.shape[0])
    refined = np.empty(times_s.shape, dtype=np.float64)

    for i, t in enumerate(times_s):
        frame_f = float(t) * fps
        frame_i = int(round(frame_f))
        if frame_i <= 0 or frame_i >= n - 1:
            refined[i] = frame_f / fps
            continue

        y0 = float(prob[frame_i - 1])
        y1 = float(prob[frame_i])
        y2 = float(prob[frame_i + 1])
        denom = (y0 - 2.0 * y1 + y2)
        if denom == 0.0:
            delta = 0.0
        else:
            delta = 0.5 * (y0 - y2) / denom
            # keep refinement local; also guards weird/noisy peaks
            if delta < -1.0:
                delta = -1.0
            elif delta > 1.0:
                delta = 1.0

        refined[i] = (frame_i + delta) / fps
...
class Spect2Frames:
    ...
    def __init__(self, checkpoint_path="final0", device="cpu", float16=False):
        ...
        if self.device.type == "privateuseone":
            _patch_rotary_embedding_for_directml()
        self.float16 = float16
        self.model = load_model(checkpoint_path, self.device)

    def spect2frames(self, spect):
        # torch-directml (privateuseone) has issues with inference tensors; use
        # no_grad() there to avoid creating inference-mode views.
        inference_ctx = (
...
class Audio2Frames(Spect2Frames):
    ...
    def __init__(self, checkpoint_path="final0", device="cpu", float16=False):
        ...
        # torch-directml (privateuseone) does not support complex dtypes required
        # for STFT; compute spectrograms on CPU and move to the model device.
        self._spect_device = (
            torch.device("cpu")
            if self.device.type == "privateuseone"
            else self.device
        )
        self.spect = LogMelSpect(device=self._spect_device)
    ...
    def signal2spect(self, signal, sr):
        ...
        signal = torch.tensor(signal, dtype=torch.float32, device=self._spect_device)
        spect = self.spect(signal)
        if spect.device != self.device:
...
class Audio2Beats(Audio2Frames):
    ...
    def __call__(self, signal, sr):
        ...
        # downbeats are moved onto beats by the postprocessor; preserve that mapping
